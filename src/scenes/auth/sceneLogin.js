...
import { disableSnackBar } from "../../redux/actions/user.actions";

import { View, Image, StyleSheet, Dimensions, Text } from "react-native";
import { Button, TextInput, Snackbar, withTheme } from "react-native-paper";
...
const SceneLogin = ({ user, ...props }) => {
  ...
  const onDismissSnackBar = () => store.dispatch(disableSnackBar());
    ...
    <DefaultPage>
      ...
      <View style={styles.loginFormContainer}>
        ...
        <TextInput
          ...
          selectTextOnFocus={true}
          selectionColor={colors.selectionColor}
          onFocus={() => setPasswordInputColor(colors.primary)}
          onBlur={() => setPasswordInputColor(colors.blurInput)}
          right={
            <TextInput.Icon
              icon={secureTextEntry ? "eye" : "eye-off"}
              color={passwordInputColor}
              style={styles.loginFormInputIconPass}
              onPress={() => changePasswordVisibility()}
            />
          }
          style={styles.loginFormInputPass}
          onChangeText={(value) => setPassword(value)}
          secureTextEntry={secureTextEntry}
        />
        <Button
          loading={user.loading}
          icon={user.isLoggedIn ? "lock-open" : "lock"}
          mode="contained"
          onPress={() => handleLogin()}
