...
} from "./../../redux/utils/tracking.functions";

const FabButton = ({ tracking, user, ...props }) => {
  // State
  ...
  const handleUploadButton = () => {
    tracking.currentRunDistance < user.distance
      ? setDialogText(
          "Su distancia no se ha completado y no contará para los resultados. ¿Está seguro de que quiere publicarla? Puede volver a correr más tarde si así lo desea."
        )
      : setDialogText(
          "Has completado tu distancia, enhorabuena. ¿Quiere publicarla?"
        );
    showDialog();
  };
  ...
  const uploadTrackInfo = () => {
    setLoading(true);
    let ajaxurl =
      "https://www.sansilvestrevalladolid.es/wp-admin/admin-ajax.php";
    ...
    <View style={styles.fabContainer}>
      <Portal>
        <Dialog visible={visible} onDismiss={hideDialog}>
          ...
          <Dialog.Actions>
            ...
            <Button onPress={uploadTrackInfo}>
              {error ? "Reintentar" : "Publicar"}
            </Button>
          </Dialog.Actions>
        </Dialog>
      </Portal>
      <Portal>
        <Dialog visible={visibleBackGround} onDismiss={hideDialogBackGround}>
          ...
          <Dialog.Content>
            <Paragraph>
              Asegúrese de que concede permisos de ubicación en segundo plano a
              esta aplicación, para ello, diríjase a ajustes de la aplicación y
              cambie los permisos de ubicación a la opción "Permitir Siempre",
              así podrá registrar su posición si bloquea o minimiza la
              aplicación.
            </Paragraph>
          </Dialog.Content>
          <Dialog.Actions>
      ...
      <FAB
        ...
        animated={true}
        icon={tracking.runStatus != "started" ? "run-fast" : "stop"}
        label={tracking.runStatus != "started" ? "Correr" : "Parar"}
        onPress={() =>
...
export default withTheme(
  connect(mapStateToProps, mapDispatchToProps)(FabButton)
);
